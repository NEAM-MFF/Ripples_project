...
# of SUA filtering and of ripple detection. It only calls
# load_block(type_sig='RB') and load_block(type_sig='LFP'), both purely
# continuous-signal loads with no reference to any spike/unit file or ripples
# dataframe anywhere. Its numeric output is identical no matter which unit
# filtering is in force, so - same as detect_ripples_df_one_arr_RS.py -
# re-running it is only necessary if you want a self-contained copy of
# everything under dataframes_new_filter/; the existing
# dataframes/delta_rb_env_analysis/ output could be reused as-is otherwise.
