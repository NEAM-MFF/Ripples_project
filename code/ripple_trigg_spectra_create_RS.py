        ...
        for array in range(1,17): 
            ...
            try:
                ...
                if df_ripp_arr.shape[0]>0:
                    ...
                    if monkey in ['N','F']:
                        LFP_arr = cut_abs_times(LFP_arr,start_t_LFP_ms,monkey,rec_type='RS',date=date,params=params_analysis)
                    ...
                    for ch in range(64):
                        ...
                        if df_ch.shape[0]>0:
                            ...
                            ### RIPPLES WITH THE PEAK SHIFTED BEFORE/AFTER
                            ...
                            if TRIGG_POINT=='peak':
                                HIGH_only_common_peaks = df_ch_high_ratio['positive_peak_time_ms'].values + arr_start 
                                LOW_only_common_peaks = df_ch_low_ratio['positive_peak_time_ms'].values + arr_start 
                                ZERO_only_common_peaks = df_ch_zero_ratio['positive_peak_time_ms'].values + arr_start 
                            elif TRIGG_POINT=='start':
                                HIGH_only_common_peaks = df_ch_high_ratio['start_time_ms'].values + arr_start
                                LOW_only_common_peaks = df_ch_low_ratio['start_time_ms'].values + arr_start
                                ZERO_only_common_peaks = df_ch_zero_ratio['start_time_ms'].values + arr_start
                            elif TRIGG_POINT=='stop':
                                HIGH_only_common_peaks = df_ch_high_ratio['stop_time_ms'].values + arr_start
                                LOW_only_common_peaks = df_ch_low_ratio['stop_time_ms'].values + arr_start
                                ZERO_only_common_peaks = df_ch_zero_ratio['stop_time_ms'].values + arr_start
                            else:
                                print('Wrong trigger option.')
                                raise SystemExit('Exitting with wrong param. error.')
