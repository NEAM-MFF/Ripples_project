...
##### PARAMETERS #####
# Deliberately reads from code/ (not code_new_filter/), matching
# SUA_filter_metrics_pkl_create.py and SUA_write_filtered_nix.py: this stage
# doesn't depend on functions_analysis_new_filter.py's patches (it reads nix
# files directly via neo, never through load_block/load_block_human), and it
# IS the process that produces the new filter rather than consuming it.
