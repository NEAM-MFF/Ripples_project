...
# NEW FILTER VERSION.
#
# Mirrors SUA_RS_prop_pkl_create_new_filter.py's approach, applied to NATIM
# recordings. Same underlying issue: the old sua_prop_all_NATIM was built on
# the earlier "good units" file, and doesn't cover units the current
# filtering (spikes_filtered/..._spikes_KS4_filtered.nix) admits. This
# produces the new-filter equivalent under a separate DF_FOLDER root, rather
# than overwriting the old one.
#
# DEPENDS ON functions_analysis_new_filter.py, same as the RS script -
# specifically the load_block() branch for type_sig='spikes_KS4_filtered'.
# For type_rec='NATIM' that branch reads
...
# (NATIM recordings are stored under the "TVSD" folder/file naming, not
# "NATIM" - already handled inside load_block, nothing to do here beyond
# passing the right type_sig). Deploy functions_analysis_new_filter.py
# alongside this file in code_new_filter/ as functions_analysis.py.
#
# WHAT CHANGED VS. THE ORIGINAL SCRIPT
#   1. Spike loading swapped from load_block(type_sig='spikes_KS4') (old
#      good-units file) to load_block(type_sig='spikes_KS4_filtered').
#   2. The filtered files carry NO per-spike waveforms, so
...
#      traceability - same as the RS script.
#   4. monkey/date added to channel_prop. These were missing in the original
#      NATIM script even though ['monkey','date','array','cell_name'] is the
#      join key used elsewhere (e.g. SF8_cliques_space_NATIM_filtered,
#      graph_SUA_preprocess). Without them cell_name collides across
#      recordings once more than one date/monkey is concatenated.
#   5. params_analysis.yml read from code_new_filter/, DF_FOLDER points at
#      dataframes_new_filter/, under the SAME subfolder names
#      (sua_prop_NATIM/, sua_prop_all_NATIM/) the rest of the pipeline
#      expects (load_prop_df takes df_folder as an argument).
#   6. Fixed a pre-existing syntax bug in the OP-map read: the original had
#      f'{params['data_folder']}/metadata/...' - identical nested quotes
#      inside an f-string, which is invalid syntax before Python 3.12. Now
#      f"{params['data_folder']}/metadata/..." (outer double quotes). Left
#      the key itself (params['data_folder'], not natim_data_folder) as it
#      was, since OP maps apparently live under the general data root rather
#      than the NATIM-specific raw data root.
#   7. Everything else - RB/LFP signal loading, OP map lookup, phase-property
#      calculation, waveform/width/final-class classification - is
#      UNCHANGED, since none of it depends on which unit set is classified.
...
DF_FOLDER = f'{MAIN_FOLDER}/dataframes_new_filter' ### NEW: separate root, see note 5 above
...
print(f"Running Spike prop. (new filter) for Monkey {monkey}, Date {date}.")
...
if calculate_SUA_prop:
    print('Calculating SUA NATIM properties, main dataframes (new filter).')
    prop_list = []
    for array in range(1,17):
        ...
        try:
            ...
            for cell in range(num_cells):
                ...
                channel_prop['train_order_original'] = int(annot(spike_train, 'train_order_original', -1))  # order in the unfiltered file, for traceability
                channel_prop['monkey'] = monkey  # NEW: needed for the ['monkey','date','array','cell_name'] join key used downstream
...
if calculate_other_prop:
    print('Calculating SUA NATIM properties - additional modification of main dataframes (new filter).')
    ...
    try:
        ...
        print(f'For date {date} the dataframe was not added. ({e})')
