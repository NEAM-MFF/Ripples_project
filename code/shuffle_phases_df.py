...
#
# NEW FILTER VERSION - this one DOES depend on the SUA filtering, on two
# separate points:
#   1. It loads spikes directly (load_block(type_sig='spikes_KS4') -> now
#      'spikes_KS4_filtered'), so it iterates over whichever unit set that
#      points at.
#   2. For every cell it looks up classification/noise columns from
#      f'{DF_FOLDER}/sua_prop_all/monkey{monkey}_all_arrays_date_{date}.pkl'
#      by cell_name - since DF_FOLDER is changed to dataframes_new_filter/
#      below, this now reads the new-filter sua_prop_all built by
#      SUA_RS_prop_pkl_create_new_filter.py, not the old good-units one. That
#      dependency is implicit (same variable), so no separate path needed -
#      just make sure that pkl already exists for every monkey/date before
#      running this, or the "Cannot open the SUA prop. file." except will
#      fire for every cell.
# It does NOT depend on ripple-detection dataframes (detect_ripples_df_one_arr_RS.py) -
# nothing here reads from a ripples/ folder.
#
# requires functions_analysis.py to be the patched version (load_block
# supporting type_sig='spikes_KS4_filtered') - see functions_analysis_new_filter.py.
...
DF_FOLDER = f'{MAIN_FOLDER}/dataframes_new_filter' ### NEW: here the resulting dataframes will be saved, and where sua_prop_all is read from
MONKEY_LIST = ['L','N','F']
        ...
        for array in range(1,17):
            ...
            try:
                ...
                for cell in range(num_cells):
                    ...
                    ### NEW: reads from DF_FOLDER = dataframes_new_filter, i.e. the
                    ### new-filter sua_prop_all, not the old good-units one
                    try:
                ...
                print(f'For array {array}, the shuffle properties were not calculated. ({e})')
