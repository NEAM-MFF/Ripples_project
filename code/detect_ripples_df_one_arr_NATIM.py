...
#
# NEW FILTER VERSION. Two changes from the original, same as
# detect_ripples_df_one_arr_RS_new_filter.py:
#   1. params_analysis.yml read from code_new_filter/.
#   2. DF_FOLDER (final destination) points at dataframes_new_filter/.
#
# ripple_prop() only calls load_block(type_sig='LFP') and
# load_block(type_sig='RB') - it never touches spike/unit files, so ripple
# detection output is identical regardless of which SUA filtering is in use.
# Re-running is not strictly necessary if you already have
# dataframes/NATIM_ripples_lowpass_40Hz_min_dur_40/ from the old run - could
# just point at / copy that instead. Writing to dataframes_new_filter/ here
# assumes a fully self-contained copy under the new pipeline's folder.
